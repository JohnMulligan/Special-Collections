...
const NetworkView = () => {
  ...
  const radii = Object.fromEntries(
    data.map((d) => [
      d["o:id"],
      (links.filter((link) => link.source === d["o:id"]).length + 1) *
        radiusFactor,
    ])
  );
  ...
  function initializeSimulation() {
    simulation.nodes(data).on("tick", tickActions);
    initializeForces();
  }
  ...
  function initializeForces() {
    simulation
      .force(
        "charge_force",
        d3.forceManyBody().strength(forceProperties.charge.strength)
      )
      .force(
        "links",
        d3.forceLink(links).id((d) => d["o:id"])
      )
      .force("center_force", d3.forceCenter(width / 2, height / 2))
  ...
  const node = g
    ...
    .on("mouseover", function (d, i) {
      ...
      textElements
        .filter(
          (node) =>
            connectedLinks.indexOf(node["o:id"]) === -1 &&
            node["o:id"] !== i["o:id"]
        )
        .style("visibility", "hidden");
    })
  ...
  const textElements = g
    ...
    .style("opacity", (node) =>
      radii[node["o:id"]] > minRadiusForLabel ? 1 : 0
    );
        ...
        .on("zoom", (e) => {
          ...
          g.selectAll("text")
            ...
            .style("opacity", (d) => {
              ...
              return e.transform.k * radii[d["o:id"]] > minRadiusForLabel
                ? 1
                : 0;
            });
        })
      ...
      <div className={classes.root}>
        <label title="Negative strength Repels nodes. Positive Strength attracts nodes">
          ...
          <output id="charge_StrengthSliderOutput">
            ({-1 * forceProperties.charge.strength})
          </output>
          <Slider
